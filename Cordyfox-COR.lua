...
function user_setup()
    ...
    state.RangedMode:options('STP', 'Normal', 'Accuracy', 'MaxAccuracy', 'Critical')
...
function init_gear_sets()
    ...
    sets.precast.RA = {
        ...
        feet = "Adhemar Gamashes +1", --10
        neck = "Comm. Charm +1", -- 3
      --back = gear.COR_SNP_Cape, --
        waist= "Impulse Belt", -- 3
    } -- 54 Snapshot (/11 Rapid Shot)
    ...
    sets.precast.WS['Last Stand'] = sets.precast.WS
        ...
        sets.precast.WS['Last Stand'].Accuracy = set_combine(sets.precast.WS['Last Stand'], {
            ammo = gear.RAccbullet,
            neck = "Iskur Gorget",
            ear1 = "Beyla Earring",
            ear2 = "Telos Earring",
    ...
    sets.precast.WS['Requiescat'] = set_combine(sets.precast.WS['Swift Blade'], {
        ...
        sets.precast.WS['Requiescat'].Accuracy = set_combine(sets.precast.WS['Requiescat'], {
            head="Meghanada Visor +2",
    ...
    sets.midcast.RA.MaxAccuracy = set_combine(sets.midcast.RA.Accuracy, {
        legs = "Laksa. Trews +3",
        ring1= "Regal Ring",
        waist= "K. Kachina Belt +1",
    })
    ...
    sets.engaged.DW.Accuracy.MaxHastePlus = set_combine(sets.engaged.DW.Accuracy.MaxHaste, {back=gear.COR_DW_Cape})
    sets.engaged.DW.MaxAccuracy.MaxHastePlus = set_combine(sets.engaged.DW.MaxAccuracy.MaxHaste, {back=gear.COR_DW_Cape})
    ...
    sets.engaged.DW.Accuracy.DT.MaxHastePlus = set_combine(sets.engaged.DW.Accuracy.MaxHastePlus, sets.engaged.Hybrid)
    sets.engaged.DW.MaxAccuracy.DT.MaxHastePlus = set_combine(sets.engaged.DW.MaxAccuracy.MaxHastePlus, sets.engaged.Hybrid)
    ...
    sets.DeathPenalty_M.Accuracy = {main={name="Rostam", bag="wardrobe3"}, sub={name="Rostam", bag="wardrobe"}, ranged="Death Penalty"}
    sets.DeathPenalty_R = {main="Lanun Knife", sub="Tauret", ranged="Death Penalty"}
    sets.DeathPenalty_R.Accuracy = {main="Lanun Knife", sub={name="Rostam", bag="wardrobe"}, ranged="Death Penalty"}
    sets.Armageddon_M = {main="Naegling", sub="Tauret", ranged="Armageddon"}
    sets.Armageddon_M.Accuracy = {main={name="Rostam", bag="wardrobe3"}, sub={name="Rostam", bag="wardrobe4"}, ranged="Armageddon"}
    sets.Armageddon_R = {main="Lanun Knife", sub="Kustawi +1", ranged="Armageddon"}
    sets.Armageddon_R.Accuracy = sets.Armageddon_R
    sets.Fomalhaut_M = {main={name="Rostam", bag="wardrobe3"}, sub="Blurred Knife +1", ranged="Fomalhaut"}
    sets.Fomalhaut_M.Accuracy = {main={name="Rostam", bag="wardrobe3"}, sub="Kustawi +1", ranged="Fomalhaut"}
    sets.Fomalhaut_R = {main="Lanun Knife", sub="Kustawi +1", ranged="Fomalhaut"}
    sets.Fomalhaut_R.Accuracy = sets.Fomalhaut_R
    sets.Ataktos = {main="Naegling", sub="Blurred Knife +1", ranged="Ataktos"}
    sets.Ataktos.Accuracy = {main="Naegling", sub="Demers. Degen +1", ranged="Ataktos"}
...
function get_custom_wsmode(spell, action, spellMap)
    local wsmode
    if spell.skill == 'Marksmanship' then
        if state.RangedMode.value == 'Accuracy' or state.RangedMode.value == 'MaxAccuracy' then
            wsmode = 'Accuracy'
        end
    else
        if state.OffenseMode.value == 'Accuracy' or state.OffenseMode.value == 'MaxAccuracy' then
            wsmode = 'Accuracy'
        end
    end
...
function check_weaponset()
    if state.OffenseMode.value == 'Accuracy' or state.OffenseMode.value == 'MaxAccuracy' or state.OffenseMode.value == 'MaxAccuracy' then
        equip(sets[state.WeaponSet.current].Accuracy)
    else
