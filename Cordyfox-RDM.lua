...
function user_setup()
    ...
    if player.sub_job == BLM then
        send_command('bind numpad+ input /ma "Sleepga" <stnpc>')
    else 
        send_command('bind numpad+ input /ma "Sleep Ii" <stnpc>')
    end
    
    send_command('bind ^numpad+ input /ma "Bind" <stnpc>')
    send_command('bind @numpad+ input /ma "Gravity" <stnpc>')
...
function init_gear_sets()
    ...
    } -- 44%/42%. Caps at 80%, 38% of which is innate to RDM.
    ...
    sets.midcast.Cure = {
        ...
        back="Aurist's Cape +1",
        waist="Luminary Sash",
    } -- 84/50% Cure Potency I, 15/30% Cure Potency II
    ...
    sets.midcast.StatusRemoval = {
        --main="Prelatic Pole" +10 skill
        --head="Vanya Hood", Path B: +20 skill
        body="Viti. Tabard +3", -- +23 skill
        legs="Atrophy Tights +2", -- +15 skill
        -- feet="Vanya Clogs", Path B: +20 skill
        neck="Incanter's Torque", -- +10 skill
        --ear2="Meili Earring", +10 skill
        ring1="Haoma's Ring", -- +8 skill, +15% Cursna
        ring2="Menelaus's Ring", -- +15 skill, +20% Cursna
        back="Perimede Cape", -- 4% Quick Cast
        --waist="Bishop's Sash", +5 skill
    }

        sets.midcast.Cursna = set_combine(sets.midcast.StatusRemoval, {
            hands="Hieros Mittens", -- +10% Cursna
            --neck="Debilis Medallion", 
            --ear1="Beatific Earring",
            back="Oretan. Cape +1", -- +5% Cursna
        }) -- +50% Cursna success rate
    ...
    sets.midcast.Stoneskin = set_combine(sets.midcast.EnhancingDuration, {
        ...
        neck="Stone Gorget", -- +30
        waist="Siegel Sash", -- +20, -8% cast
    }) 
    ...
    sets.magic_burst = {
        ...
        body="Bunzi's Robe", --9/(9)
        hands="Amalric Gages +1", --(6)
        legs="Bunzi's Pants", --8/(8)
        feet="Bunzi's Sabots", --5/(5)
    ...
    sets.engaged.MaxAccuracy = set_combine(sets.engaged, {
        ammo="Voluspa Tathlum",
        head="Carmine Mask +1",
        body="Carm. Sc. Mail +1",
...
function job_buff_change(buff,gain)
	...
	if buff == "charm" then
        if gain then
            send_command('@input /p <me> is Charmed! Put me to Sleep!')
        else
            send_command('@input /p No longer Charmed!')
            handle_equipping_gear(player.status)
        end
    end
...
organizer_items = {
    ...
    naegling="Naegling",
    tauret="Tauret",
    gletis="Gleti's Knife",
    lowdmg1="Ceremonial Dagger",
    lowdmg2="Qutrub Knife",
    contemplator="Contemplator +1",
    ...
    -- AF
    artifact_head="Atro. Chapeau +2",
    artifact_body="Atrophy Tabard +3",
    artifact_hands="Atrophy Gloves +3",
    artifact_legs="Atrophy Tights +2",
    artifact_feet="Atrophy Boots +1",
    -- Relic
    relic_head="Viti. Chapeau +3",
    relic_body="Viti. Tabard +3",
    relic_hands="Viti. Gloves +3",
    relic_legs="Viti. Tights +3",
    relic_feet="Vitiation Boots +3",
    -- Empyrean
    empy_head="Leth. Chappel +1",
    empy_body="Lethargy Sayon +1",
    empy_hands="Leth. Gantherots +1",
    empy_legs="Leth. Fuseau +1",
    empy_feet="Lethargy Houseaux +1",
}